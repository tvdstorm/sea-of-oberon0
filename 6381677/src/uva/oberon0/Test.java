...
public class Test 
{
	...
	private static void runTestExpression(String name, String text, int resultExpected)
	{
	...
	public static void testExpressions()
	{
		...
		runTestExpression("Expression Calc Division", "(4+(2+2)) DIV (2+(1+1))", 2);
	...
	public static void testStatements()
	{
		...
		runTest("Statement Array Assign with Int Index", "VAR i: ARRAY 1 OF INTEGER; BEGIN i[0]:=1; r:=(i[0] = 1) END");
		runTest("Statement Array Assign with Var Index", "VAR i: ARRAY 2 OF INTEGER; VAR i2,x: INTEGER; BEGIN i2:=1; x:=10; i[i2]:=x; r:=(i[i2] = x) END");

		runTest("Statement IF", "IF 1=1 THEN r:=(1) END");
		runTest("Statement IF with ELSE", "IF 1=1 THEN r:=(1) ELSE r:=(0) END");
		runTest("Statement IF return on ELSE", "IF 1=0 THEN r:=(0) ELSE r:=(1) END");
		runTest("Statement IF with ELSIF", "IF 1=1 THEN r:=(1) ELSIF 1=0 THEN r:=(0) END");
		runTest("Statement IF return on ELSIF", "IF 1=0 THEN r:=(0) ELSIF 1=1 THEN r:=(1) END");
		runTest("Statement IF with ELSIF and ELSE", "IF 1=1 THEN r:=(1) ELSIF 1=0 THEN r:=(0) ELSE r:=(0) END");
		runTest("Statement IF with ELSIF and ELSE", "IF 1=0 THEN r:=(0) ELSIF 1=1 THEN r:=(1) ELSE r:=(0) END");
		runTest("Statement IF with ELSIF and ELSE", "IF 1=0 THEN r:=(0) ELSIF 0=1 THEN r:=(0) ELSE r:=(1) END");

		runTest("Statement WHILE", "VAR i: INTEGER; BEGIN i:=0; WHILE i<3 DO i:=i+1 END; r:=(i = 3) END");
		runTest("Statement WHILE", "VAR i,i2: INTEGER; BEGIN i:=0; i2:=3; WHILE i<i2 DO i:=i+1 END; r:=((i = 3) & (i = i2)) END");
	}
