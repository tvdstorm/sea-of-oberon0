...
public class GlobalProcedures {
	...
	public static void initGlobalProcedures(Environment environment,
			final BufferedReader input, final PrintWriter output) {
	...
	private static void initRead(Environment environment,
			...
			public void evaluate(Environment environment) {
				...
				try {
					...
					environment.assignValue(
							fPName,
							new com.kootsjur.oberon.value.Int(Integer
									.parseInt(line)));
		...
		Procedure procedure = new Procedure(formalParameters, declarations,
				procedures, statementSequence, environment);
		environment.declareProcedure(procedureName, procedure);

	}

	private static void initWrite(Environment environment,
		...
		Statement statement = new Statement() {
			@Override
			public void evaluate(Environment environment) {
				com.kootsjur.oberon.value.Int y = (com.kootsjur.oberon.value.Int) environment
						.lookUpValue(fPName);
	...
	private static void initWriteLn(Environment environment,
			final PrintWriter output) {
		FormalParameters formalParameters = new FormalParameters();
		List<Declaration> declarations = new LinkedList<Declaration>();
		List<ProcedureDeclaration> procedures = new LinkedList<ProcedureDeclaration>();
