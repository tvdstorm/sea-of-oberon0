...
public class FPSection {
	...
	public FPSection(List<String> names, TypeDefinition typeDefinition,
	...
	public void declare(Environment environment) {
		...
		for (String parameterName : names) {
			Parameter parameter = new Parameter(typeDefinition,
					parameterDirection);
		...
		// post-condition
		for (String parameterName : names) {
			assert (environment.getDeclaredParameters()
					.containsKey(parameterName)) : "Error in ConstantDeclaration method declare! Constant is not declared!";
		}
	}
}
