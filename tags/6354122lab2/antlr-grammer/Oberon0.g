...
fpSection
  : (var='VAR')? fieldList
    { 
      ...
      for ( FieldNode node : $fieldList.return_fieldlist){
         $formalParameters::fields.add(new FormalParameterNode(node.getName(), node.getType(), callByRef));
      }
    }
  ;

formalParameters returns[List<FormalParameterNode> formalParams]
scope{
  List<FormalParameterNode> fields
}
@init{
  $formalParameters::fields = new LinkedList<FormalParameterNode>();  
}
  :
  '(' 
  fpSection (';' fpSection)* 
