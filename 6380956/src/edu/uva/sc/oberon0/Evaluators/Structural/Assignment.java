...
public class Assignment implements IStatement {
	...
	public Object evaluate(IScope scope) {
		...
		if(leftRef!=null && VariableRef.IsMyType(leftRef)){
			//if(((VariableRef)leftRef).selector != null && ((VariableRef)leftRef).selector.GetChildSelector() == null) {
			if(((VariableRef)leftRef).selector != null && selector != null) {
				((VariableRef)leftRef).selector.SetChildSelector(selector);
			}
