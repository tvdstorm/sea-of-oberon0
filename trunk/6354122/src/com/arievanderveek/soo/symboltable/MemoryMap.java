...
public class MemoryMap {
	...
	public MemoryAddress copyValueToNewAdress(MemoryAddress oldAddress)
	...
	private MemoryAddress generateMemoryAdress() throws SeaOfOberonException {
		...
		MemoryAddress address = new MemoryAddress(
				generator.nextInt(RANDOM_MAX_INT));
		int retryCounter = 0;
		while (memoryMap.containsKey(address)) {
			...
			if (retryCounter > 5) {
				throw new SeaOfOberonException(
						"Could not generate memory address after "
