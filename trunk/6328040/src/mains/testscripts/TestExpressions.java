...
public class TestExpressions {
	...
	public static void main(String[] args) {
		...
		assert (4 == ((IntegerValue) new AddNode(int1, int3).eval(context))
				.getValue());
		assert (-3 == ((IntegerValue) new SubtractNode(int0, int3)
				.eval(context)).getValue());
		assert (12 == ((IntegerValue) new MultiplyNode(int4, int3)
				.eval(context)).getValue());
		assert (2 == ((IntegerValue) new DivideNode(int4, int2).eval(context))
				.getValue());
		assert (1 == ((IntegerValue) new ModuloNode(int4, int3).eval(context))
				.getValue());
		assert (-7 == ((IntegerValue) new NegativeNode(new AddNode(int4, int3))
				.eval(context)).getValue());

		System.out.print("Arithmetic operations passed\n");

		assert (!((BooleanValue) new EqualsNode(int4, int3).eval(context))
				.getValue());
		assert (((BooleanValue) new EqualsNotNode(int3, int4).eval(context))
				.getValue());
		assert (!((BooleanValue) new GreaterThenNode(int3, int4).eval(context))
				.getValue());
		assert (!((BooleanValue) new GreaterEqualNode(int3, int4).eval(context))
				.getValue());
		assert (((BooleanValue) new SmallerThenNode(int3, int4).eval(context))
				.getValue());
		assert (((BooleanValue) new SmallerEqualNode(int3, int4).eval(context))
				.getValue());

		System.out.print("Relation operations passed\n");

		assert (false == ((BooleanValue) new ConjunctionNode(boolfalse,
				booltrue).eval(context)).getValue());
		assert (true == ((BooleanValue) new DisjunctionNode(boolfalse, booltrue)
				.eval(context)).getValue());
		assert (true == ((BooleanValue) new NegationNode(boolfalse)
				.eval(context)).getValue());
