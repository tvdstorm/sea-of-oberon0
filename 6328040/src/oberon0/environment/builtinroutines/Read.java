...
public class Read implements ICallable {
	...
	@Override
	/*
	 * Reads a number from the console and returns it to the first reference in
	 * actual parameters
	 */
	public void call(Context context, List<IReferable> actualParameters) {
		...
		Reference ref = actualParam.getRef(context,
				IReferable.noPreviousReference);
		...
		while (!validInputReceived) {
			try {
				...
				System.err.print("Not a valid number " + line
						+ " please try again: ");
			} catch (IOException e) {
				System.err.print("Unexpected IO ERROR: " + e
						+ " please try again: ");
			}
		}
